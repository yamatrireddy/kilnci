...
| T-09 | I | Stored XSS through logs, PR titles, commit messages, or markdown | H | H | **High** | React escaping, sanitized ANSI log renderer, markdown sanitizer, strict CSP (SS §6, §10) | I (React escaping, strict CSP, ESLint/Semgrep bans; untrusted run titles/branches/step names have control and Unicode format characters (bidi overrides, zero-width) replaced); log viewer: I (only SGR color/style escapes are interpreted and map to fixed classes; OSC links and titles, cursor and other escapes, C0/C1 controls dropped; bidi overrides replaced; escape strings bounded and ended by any new ESC; lines capped at 16 384 characters and 256 styled spans, views at 10 000 lines / 4M characters with 2 000 rendered at a time; run titles, branches, and job names shown through `displayText` (bidi and controls replaced): `packages/core/src/ansi.test.ts`, "never turns log bytes into markup, links, or styles" in `packages/ui/src/runs.test.tsx`. The viewer is a lossy, normalized view; the raw log endpoint is the record) |
| T-10 | D | Webhook floods, huge payloads, log-stream fan-out exhaust resources | H | M | **High** | Body size limits, rate limits per IP/principal, per-org quotas, async webhook queue (SS §5, §12) | I (body limits, per-IP /64 + per-principal rate limits; log streams and downloads capped per user across all of the user's credentials, per org (after authorization), and in total, streams capped in duration (30 min): TestLogLimiter_*. Clients back off on stream failures (a stream counts as healthy only after output or 30 s open), honor `Retry-After`, and give up after 6 failures; clients without streaming re-read the stored log every 30 s, not faster. Residual: the total caps (2000 streams, 128 downloads) are shared by all tenants, so a few orgs can exhaust them; log chunks 256 KiB and 64 MiB per job attempt: TestAppend_TruncatesAtTheLimit); webhook queue: Phase 1 slice 8 |
...
| T-14 | T | Compromised runner forges job results (marks failing tests as passed, swaps artifacts) | M | H | **High** | Results tied to lease; artifact digests recorded; provenance attestations signed server-side; protected deploys require trusted runners (SS §8) | I (results bound to lease ID + runner + unexpired lease: TestLease_BoundToRunnerAndLeaseID; log chunks scoped to the lease attempt, lease re-checked under the row lock, writer recorded: TestAppend_EachAttemptHasItsOwnLog); artifact digests/provenance: Phase 3 |
...
| T-19 | E | Container escape to runner host (privileged, Docker socket, kernel exploit) | M | C | **High** | Non-root, no privileged, caps dropped, seccomp/AppArmor, no Docker socket, rootless BuildKit, optional gVisor/Kata, ephemeral runners (SS §8) | I (non-root 65532:65532 (not a host user's UID; userns-remap recommended), cap-drop ALL, no-new-privileges, default seccomp/AppArmor, init, no host namespaces/mounts/Docker socket: TestDocker_SandboxIsHardened, TestHardenedHostConfig); gVisor/Kata: later |
| T-20 | E | Job reaches cloud metadata service and steals node credentials | H | C | **Critical** | NetworkPolicy/iptables block metadata IPs, IMDSv2 hop limit 1, no node IAM roles on runner nodes | P (host firewall rules documented in docs/runners.md; the runner refuses to start without --egress-policy acknowledgement, and `none` stamps a warning on every job log) |
...
| T-23 | I | Workspace or cache leaks data between jobs/projects | M | H | **High** | Workspace wiped per job, per-project cache namespaces, ephemeral runners recommended | I (per-job network without ICC and per-job volume, all removed after the job: TestDocker_CleansUpEverything; clone credential never in the workspace: TestDocker_ChecksOutExactCommit) |
...
| T-26 | D | Crypto-mining, fork bombs, or disk filling exhaust runner resources | H | M | **High** | CPU, memory, PID, disk, and time limits; per-org concurrency quotas | I (memory without swap, CPU, PID limits, step and job timeouts in the runner plus the server reaper: TestDocker_StepTimeoutAndCancel, TestHeartbeat_ReportsCancelAndTimeout; per-job disk limit on the container layer and workspace volume, runner fails closed where Docker cannot enforce it: TestHardenedHostConfig, TestDocker_DiskLimit). Residual: the limit needs overlay2 on XFS with pquota, and `--job-disk-limit=off` accepts unlimited disk use; image pull size is not capped; per-org concurrency quotas: later |
| T-27 | I | Secret masking bypass (split output, char-by-char, alternate encodings) | H | H | **High** | Masking of base64/URL/hex variants, streaming masker handling chunk boundaries; documented that masking is a safety net, not a boundary — T-18 controls are primary | I (runner-side streaming masker incl. Authorization header credential parts; base64 in 3 alignments, also wrapped at any width of 16+ columns and any offset; lower/upper hex; URL; JSON-escaped; per-line; output independent of how writes are split: FuzzMask, FuzzMask_SplitInvariant, TestAgent_RunsJobMasksOutputAndReports). Residual: values under 4 bytes (Phase 2 secret creation must reject or warn), base64 wrapped under 16 columns, up to 3 characters at a wrapped line edge, other encodings; masking is bounded by refusing jobs whose values exceed 64 KiB each or 256 KiB in total (TestCheck_Limits, TestAgent_RefusesJobWhoseValuesAreTooLargeToMask), and wrapped base64 is masked only for values up to 4 KiB; the Phase 2 secrets API must enforce the same limits |
| T-60 | E | Job image reference points at a registry on the runner host or its network (`127.0.0.1:2375/x`, `10.0.0.5:8080/x`, metadata IPs), so `dockerd` makes requests from the host network, outside the job's egress rules | M | M | **Medium** | Refuse images from loopback, private, link-local, CGNAT, and metadata registries (literal or resolved, fail closed on lookup errors); exact operator allow-list; restrict `dockerd` egress | I (TestValidate_ImageRegistry, TestRun_BlockedRegistryFailsJobWithReason). Residual: `dockerd` re-resolves the name (DNS rebinding); a public registry's token realm or blob redirects can still point `dockerd` at internal addresses (blind requests; pull errors reach only the runner log); daemon-configured mirrors and NAT64 prefixes are not checked. Restricting `dockerd`'s own egress is the real control |
...
| T-61 | I/T | `kiln` CLI: API token leaks (argv, shell history, errors, redirects, clear text), or attacker-controlled pipeline text echoed by the lint API injects terminal escapes or bidi reordering into a developer's terminal or CI log | M | M | **Medium** | Token only from `KILN_TOKEN` or a file/piped stdin, never argv, never printed; https only (http for loopback); TLS verified; redirects not followed; problem text stripped of control and format characters (escaped in JSON); lint-only scoped tokens recommended ([cli.md](cli.md)). Residual: the token lives in the CI secret store or environment, not an OS keychain | V (TestLoad, TestLint_RedirectDoesNotSendToken, TestWrite_Text, TestWrite_JSON, TestClean) |
