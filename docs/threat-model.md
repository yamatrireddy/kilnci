...
| T-19 | E | Container escape to runner host (privileged, Docker socket, kernel exploit) | M | C | **High** | Non-root, no privileged, caps dropped, seccomp/AppArmor, no Docker socket, rootless BuildKit, optional gVisor/Kata, ephemeral runners (SS §8) | I (non-root 1000:1000, cap-drop ALL, no-new-privileges, default seccomp/AppArmor, init, no host namespaces/mounts/Docker socket: TestDocker_SandboxIsHardened, TestHardenedHostConfig); gVisor/Kata: later |
